...
with raw as (
  select
    ...
    -- bad 'YYYY.MM.DD' → NULL instead of ERROR
    try_to_date(
      VARIANT_COL:date_played::string,
      'YYYY.MM.DD'
    )                                                   as date_played,

    -- same for timestamp
    try_to_timestamp_ntz(
      VARIANT_COL:date_created::string,
      'YYYY-MM-DD"T"HH24:MI:SS+0000'
    )                                                   as date_created,

    VARIANT_COL:white::string                           as white_player,
    try_cast(nullif(trim(VARIANT_COL:white_elo::string), '') as number)
                                                       as white_elo,
    VARIANT_COL:white_title::string                     as white_title,

    VARIANT_COL:black::string                           as black_player,
    try_cast(nullif(trim(VARIANT_COL:black_elo::string), '') as number)
                                                       as black_elo,
    ...
    try_cast(nullif(trim(VARIANT_COL:winner_elo::string), '') as number)
                                                       as winner_elo,
    try_cast(nullif(trim(VARIANT_COL:loser_elo::string), '') as number)
                                                       as loser_elo,
    try_cast(nullif(trim(VARIANT_COL:winner_loser_elo_diff::string), '') as number)
                                                       as elo_diff,
...
where game_id      is not null
  and date_played  is not null   -- now strips out bad dates
